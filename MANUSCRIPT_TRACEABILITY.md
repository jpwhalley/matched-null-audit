...
| §2.4 exclusion of clustering metrics on precision (cluster-ARI band 43% of baseline against 0.7% for macro-F1) | `analysis/E7_cluster_metric_diagnostic.py` | `outputs/E2_ablation_<ds>.json`, `outputs/E2_baseline_<ds>.json` | `outputs/E7_cluster_metric_diagnostic.{csv,json}` | local |
...
| *k*-sweep at *k*=25 and *k*=100 — **shipped but deliberately not reported.** No *k*-specific matched nulls were run, so the point estimates cannot be interpreted and no manuscript claim rests on them | `analysis/E2_downstream_ablation.py --ablation` | as above | `k_sensitivity` block in `outputs/E2_verdict_pbmc3k.json` | checkpoint |
